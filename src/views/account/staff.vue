<template>
    <div class="app-container account_staff">
        <el-form ref="form" :model="form" label-width="100px">
            ...
            <dir class="_table">
                <i class="addBtn el-icon-plus" @click="tianjiaTk()"></i>
                <el-table :key="tableKey" :data="list" fit style="width: 100%;">
                    ...
                    <el-table-column label="员工岗位" min-width="100" align="center">
                        <template slot-scope="scope">
                            <span
                                ...
                                v-if="scope.row.position==item.id"
                            >{{item.name}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" align="center" width="420">
                        <template slot-scope="scope">
                            ...
                            >修改</el-button>
                            <el-button size="mini" type="success" @click="cakanYg(scope.row.id)">查看</el-button>
                            <el-button
                                size="mini"
                                type="danger"
                                @click="chongshe(scope.row.id,scope.row.phone)"
                            >重设密码</el-button>
                            <el-button
                                v-if="scope.row.is_freeze==0"
                                size="mini"
                                type="danger"
                                @click="dongjie(scope.row.id,scope.row.is_freeze,scope.row)"
                            >冻结</el-button>
                            <el-button
            ...
            >
                <div class="left">
                    ...
                    <el-form-item label="岗位" prop="position">
                        <el-select v-model="addData.position" clearable class="filter-item">
                            <el-option
                                v-for="item in GWlist"
                                :key="item.id"
                                :label="item.name"
                   ...
                   <el-form-item label="入职时间" prop="entryTime">                   
                        <el-date-picker v-model="addData.entryTime" unlink-panels type="date" placeholder="选择日期"></el-date-picker>
                    </el-form-item>
	          ...
	          <el-select v-model="addData.station_id" clearable class="filter-item">
	            <el-option v-for="item in YZlist" :key="item.id" :label="item.shortName" :value="item.id"/>
	          </el-select>
                    </el-form-item>-->
                    ...
                    <el-form-item label="生日">
                        <el-date-picker disabled v-model="addData.birthday" type="date" placeholder=""></el-date-picker>
                    </el-form-item>
        ...
        <el-dialog :visible.sync="yuangongxiugai" title="员工信息修改" class="ygtj_tab">
            ...
            >
                <div class="left">
                    ...
                    <el-form-item label="岗位" prop="position">
                        <el-select v-model="ygXiang.position" clearable class="filter-item">
                            <el-option
                                v-for="item in GWlist"
                                :key="item.id"
                                :label="item.name"
                    ...
                    <el-form-item label="入职时间" prop="entryTime">
                        <el-date-picker v-model="ygXiang.entryTime" type="date" placeholder="选择日期"></el-date-picker>
                    </el-form-item>

                    <el-form-item label="在职状态" prop="status">
                        <el-select
                            v-model="ygXiang.status"
                            clearable
                            class="filter-item"
                            @change="gaiqiyong2"
                        >
                ...
                <div class="right">
                    ...
                    <!-- <el-form-item label="所属油站">
                        <el-select v-model="ygXiang.belong_id" clearable class="filter-item">
                            <el-option
                    ...
                    <el-form-item label="生日">
                        <el-date-picker disabled v-model="ygXiang.birthday" type="date" placeholder=""></el-date-picker>
                    </el-form-item>
                    <el-form-item label="手机" prop="phone">
        ...
        <el-dialog :visible.sync="yuangongChakan" title="员工信息修改" class="ygtj_tab">
            ...
            >
                <div class="left">
                    <el-form-item label="员工编号：">{{ygChaXiang.employee_id}}</el-form-item>
                    <el-form-item label="员工姓名：">{{ygChaXiang.name}}</el-form-item>
                    <el-form-item label="身份证号：">{{ygChaXiang.idCard}}</el-form-item>
                    <el-form-item label="电话：">{{ygChaXiang.phone}}</el-form-item>
                </div>
                <div class="right">
                    <el-form-item label="岗位：">
                        <span
                            v-for="item in GWlist"
                            v-if="ygChaXiang.position==item.id"
                        >{{item.name}}</span>
                    </el-form-item>
                    ...
                    <el-form-item label="生日：">
                        <span
                            v-if="ygChaXiang.birthday"
                        >{{ygChaXiang.birthday|parseTime('{y}-{m}-{d}')}}</span>
                    </el-form-item>

                    <el-form-item label="入职时间：">
                        <span
                            v-if="ygChaXiang.entryTime"
                        >{{ygChaXiang.entryTime|parseTime('{y}-{m}-{d}')}}</span>
                    </el-form-item>
...
export default {
    ...
    methods: {
        ...
        dongjie(idid, is_freeze, row) {
            ...
            if (is_freeze == 0) {
                is_freeze = 1;
            } else if (is_freeze == 1 && row.status==0) {
                is_freeze = 0;
                is_freeze==1
            }
            let data = {
        ...
        xiugaiYg(idid) {
            this.$refs["dataForm2"].validate(valid => {
                if (valid) {
                    ...
                    let station_id = this.ygXiang.belong_id;
                    let data = {
                        id: this.ygXiangId,
                        ...
                        belong_id: this.ygXiang.belong_id,
