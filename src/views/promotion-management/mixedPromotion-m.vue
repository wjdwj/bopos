      ...
      if(this.frequency==0){
       ...
       if(this.promotionType==13){
        for(let i=0;i<this.ruleData.length;i++){
          if (this.ruleData[i].oils==''||this.ruleData[i].oils==null) {
            this.$message({
          ...
          if (this.ruleData[i].giftGoods[0].goodsName==''||this.ruleData[i].giftGoods[0].goodsName==null) {
            this.$message({
        ...
        for(let i=0;i<this.ruleData.length;i++){
          ...
          if (this.ruleData[i].giftAmount<=0) {
            this.$message({
              message: '第'+(i+1)+'条规则，请输入正确的赠送商品金额',
              type: 'warning'
            });
            return
          }
