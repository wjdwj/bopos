...
<div class="warp">
  <div class="header" style="display:flex;justify-content:  space-between;">
    <h3><span></span>促销管理</h3>
       <div class="shenqing">
        <a href="#/marketing/oilApplication-m"><el-button type="primary">油品促销下发</el-button></a>
        <a href="#/marketing/commodityApplication-m"><el-button type="primary">商品促销下发</el-button></a>
        <a href="#/marketing/mixedPromotion-m"><el-button type="primary">油商互动下发</el-button></a>
      </div>
	</div>
  <div class="main">
    <div class="top">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <!-- <span v-if="scope.row.status==-1">审批不通过</span> -->
  ...
  <el-dialog :visible.sync="dialogFormVisible" title="查看" width="80%">
    <div class="chakan">
      ...
      <div class="box">
        <span>促销时间：</span>
        <p><span>日期：{{seeData.startDateTime}} —— {{seeData.endDateTime}}</span></p>
        <p>
          <span>周期： 
            <span v-if="seeData.daysWeeks[0]=='每天'">每天</span>
            <span v-if="seeData.daysWeeks[0]!='每天'">
              周循环：
              <span v-for="(item, index) in seeData.daysWeeks" :key="index">{{item}} , </span>
            </span>
          </span>
        </p>
        <p><span>时间段：{{seeData.star}} —— {{seeData.end}}</span></p>
      </div>
      <div class="box">
        <span>促销次数：</span>
    ...
    <div class="biao">
      <div class="table">
        ...
        <el-table
          ...
          align="center" >
            <template slot-scope="scope">
              <span class="shuoming">{{scope.row.goodsName}} 加油满{{ scope.row.minimumCharge }}升，单价立减{{scope.row.discount}}元</span>         
            </template>
          </el-table-column>
        </el-table>
        <!-- 油品满升数，单价百分比优惠 -->
        <el-table
          ...
          <el-table-column label="规则说明" min-width="265" align="center" >
            <template slot-scope="scope">
              <span class="shuoming" >{{scope.row.goodsName}} 加油满{{ scope.row.minimumCharge }}升，单价优惠{{scope.row.discount}}%</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 油品满金额，总价立减 -->
        <el-table
          ...
          <el-table-column 
            ...
            align="center" >
            <template slot-scope="scope">
              <span class="shuoming" >{{scope.row.goodsName}} 加油满{{ scope.row.minimumCharge }}元，总价立减{{scope.row.discount}}元</span>
            </template>
          </el-table-column>
          <el-table-column
            label="操作"
        ...
        <el-table
          ...
          <el-table-column label="规则说明" min-width="265" align="center" >
            <template slot-scope="scope">
              <span class="shuoming"  >{{scope.row.goodsName}} 加油满{{ scope.row.minimumCharge }}元，总价优惠{{scope.row.discount}}%</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 商品满数量，单价立减 -->
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming" >{{ scope.row.goodsName }} 购买{{scope.row.minimumCharge}}个以上，单价立减{{scope.row.discount}}元</span>
            </template>
          </el-table-column>
        </el-table>
        ...
        <el-table
          ...
          <el-table-column label="说明" min-width="305px" align="center" >
            <template slot-scope="scope">
             <span class="shuoming">{{ scope.row.goodsName }} 购买达到{{scope.row.minimumCharge}}个，总价立减{{scope.row.discount}}元</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 商品单价立减 -->
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming">{{ scope.row.goodsName }}  单价立减{{scope.row.discount}}元</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 商品卖赠促销 -->
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming">购买 {{ scope.row.goodsName }}   满{{scope.row.discount}}，赠送{{scope.row.givingNum}}个{{scope.row.givingName}}</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 商品订单满金额，总价立减 -->
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming" >订单满{{scope.row.minimumCharge}}元，立减{{scope.row.discount}}元</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 商品订单满金额，总价百分比优惠 -->
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming" >订单满{{scope.row.minimumCharge}}元，优惠{{scope.row.discount}}%</span>
            </template>
          </el-table-column>
           <el-table-column
            label="操作"
        ...
        <el-table
          ...
          <el-table-column label="说明" min-width="305" align="center" >
            <template slot-scope="scope">
             <span class="shuoming">订单满{{scope.row.minimumCharge}}元，赠送{{scope.row.discount}}个{{scope.row.givingName}}</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 最低价免费 -->
        <el-table
          ...
          <el-table-column label="说明" align="center" >
            <template slot-scope="scope">
             <span class="shuoming" >购买商品组内任意商品达到{{scope.row.minimumCharge}}个，那么本次组合商品中最低价商品免费</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 油品满金额，赠送指定数量的便利店商品 -->
        <el-table
          ...
          <el-table-column label="规则说明" min-width="305" align="center" >
            <template slot-scope="scope">
              <span class="shuoming"> {{ scope.row.goodsName }} 加油满{{ scope.row.minimumCharge }}元，赠送 {{scope.row.givingNum}}个 {{scope.row.givingName}}</span> 
            </template>
          </el-table-column>
        </el-table>
        <!-- 油品满金额，赠送指定金额任意商品 -->
        <el-table
          ...
          <el-table-column label="规则说明" min-width="305" align="center" >
            <template slot-scope="scope">
              <span class="shuoming"  > {{scope.row.goodsName}} 加油满{{ scope.row.minimumCharge }}元，赠送 {{scope.row.discount}}元便利店商品</span> 
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" v-show="seeData.status==0" @click="ent">确认</el-button>
      <el-button type="primary" v-show="seeData.status==0" @click="jujueVisible = true">拒绝</el-button>
