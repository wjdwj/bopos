  ...
  <div class="app-container supplier">
    <el-form ref="form" :model="form" label-width="130px">
      <div class="top">
        ...
        <el-button type="primary" @click="handleAdd()">添加油品供应商户</el-button>
      ...
      <el-table :key="tableKey" :data="list" fit highlight-current-row class="list">
        <el-table-column label="供应商类型" align="center" min-width="100">
        ...
        <el-table-column label="是否对商户可见" min-width="110" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.isDisplay==1">是</span>
            <span v-if="scope.row.isDisplay==0||!scope.row.isDisplay">否</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" min-width="210" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            
            <el-button type="info" size="mini" @click="supplierW(scope.row.id)">修改</el-button>
            
            <el-button type="primary" size="mini" @click="chakanTk(scope.row.id)">查看</el-button>

            <el-button v-if="scope.row.status==1" size="mini" type="danger" @click="zhuangtai(0,scope.row.id,scope.row.supplierType,scope.row.supplierName)">停用</el-button>
            <el-button v-if="scope.row.status==0" size="mini" type="primary" @click="zhuangtai(1,scope.row.id,scope.row.supplierType,scope.row.supplierName)">启用</el-button>
            
            <el-button type="success" size="mini" @click="tiaozhuan(scope.row.id)" v-if="scope.row.supplierType==1">
              	维护
            </el-button>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="油品供应商添加" class="oilTianjia">
      <el-form ref="oilForm" :rules="rules" :model="oiltemp" label-position="right" label-width="135px">
        <div class="up">
          <div class="left">
            <el-form-item label="公司名称" prop="supplierName">
              <el-input v-model="oiltemp.supplierName" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="营业注册号" prop="registrationNum">
              <el-input v-model="oiltemp.registrationNum" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="法人" prop="legalPerson">
              <el-input v-model="oiltemp.legalPerson" placeholder="请输入内容"/>
            </el-form-item>
          ...
          <div class="right">
            <el-form-item label="公司简称" prop="supplierShort">
              <el-input v-model="oiltemp.supplierShort" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="联系电话" prop="phone">
              <el-input v-model="oiltemp.phone" placeholder="请输入内容" maxlength="11"/>
            </el-form-item>
            <el-form-item label="邮箱" prop="email">
              <el-input v-model="oiltemp.email" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="地址" prop="address">
              <el-input v-model="oiltemp.address" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="经度" prop="longitude">
              <el-input v-model="oiltemp.longitude" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="纬度" prop="latitude">
              <el-input v-model="oiltemp.latitude" placeholder="请输入内容"/>
            </el-form-item>
            <el-form-item label="营业执照" prop="email">
              ...
              <el-upload
								...
								>
								<img v-if="oiltemp.licenceImg" :src="oiltemp.licenceImg" class="avatar">
      ...
      <el-form ref="oilForm" :rules="rules" :model="oiltemp" label-position="right" label-width="135px">
        <div class="up">
          <div class="left">
            ...
            <el-form-item label="是否对商户可见">
              <el-switch
							  v-model="oiltemp.isDisplay"
							  active-color="#13ce66"
    ...
    <!--供应商修改-->
    <el-dialog :visible.sync="supplierFormVisible" title="供应商信息修改" class="supplierForm oilTianjia">
      <el-form ref="upForm" :rules="rules" :model="conten" label-position="right" label-width="125px">
        <div class="up">
          <div class="left">
            <el-form-item label="公司名称" prop="supplierName">
            ...
            <el-form-item label="营业执照有效期" prop="licenceValidity">
              <el-date-picker
						...
						<el-form-item label="是否对商户可见" v-if="conten.supplierType==2">
              <el-switch
							  v-model="conten.isDisplay"
							  active-color="#13ce66"
							  inactive-color="#ff4949"
							  active-text="是"
  							inactive-text="否"
          ...
          <div class="right">
            ...
            <el-form-item label="联系电话" prop="phone">
              <el-input v-model="conten.phone" maxlength="11"/>
            </el-form-item>
            <el-form-item label="邮箱" prop="email">
            ...
            <el-form-item label="经度" prop="longitude" v-if="conten.supplierType == 1">
              <el-input v-model="conten.longitude"/>
            </el-form-item>
            <el-form-item label="纬度" prop="latitude" v-if="conten.supplierType == 1">
              <el-input v-model="conten.latitude"/>
            </el-form-item>
            <el-form-item label="营业执照">
              <el-upload
								...
								>
								<img v-if="conten.licenceImg" :src="conten.licenceImg" class="avatar">
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
        ...
        <div class="up">
          ...
          <div class="right">
            ...
            <el-form-item label="地址">
             ...
             <el-form-item label="经度" v-if="conten.supplierType == 1">
              {{conten.longitude}}
            </el-form-item>
             <el-form-item label="纬度" v-if="conten.supplierType == 1">
              {{conten.latitude}}
            </el-form-item>
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        registrationNum: [{ required: true, message: '不能为空', trigger: 'blur' }],
        legalPerson: [{ required: true, message: '不能为空', trigger: 'blur' }],
        businessScope: [{ required: true, message: '不能为空', trigger: 'blur' }],
        licenceValidity: [{ required: true, message: '不能为空', trigger: 'blur' }],
        supplierShort: [{ required: true, message: '不能为空', trigger: 'blur' }],
        phone: [{ required: true, message: '不能为空', trigger: 'blur' }],
        email: [{ required: true, message: '不能为空', trigger: 'blur' },{ type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change'] }],
        address: [{ required: true, message: '不能为空', trigger: 'blur' },{ message: '请输入地址', trigger: ['blur', 'change'] }],
        longitude: [{ required: true, message: '不能为空', trigger: 'blur' },{ message: '格式不正确', trigger: ['blur', 'change'] }],
        latitude: [{ required: true, message: '不能为空', trigger: 'blur' },{ message: '格式不正确', trigger: ['blur', 'change'] }]
...
//				    this.$refs['oilForm'].resetFields();
	    ...
	    supplierW(idid) {
	    	...
	    	chakan(data).then(response=>{
	    		console.log(response)
					...
					let date_value=date.getFullYear() + '-' + this.formatTen(date.getMonth() + 1) + '-' + this.formatTen(date.getDate());
	    ...
	    tiaozhuan(idid){
			...
			addhandleAvatarSuccess(res, file) {
				...
				var that = this
				axios.post("http://shiy.wsuns.com/upload/jquery/cross/iframeupload",formData).then(function(response) {
