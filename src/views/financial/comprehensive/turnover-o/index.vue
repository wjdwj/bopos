          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <span>{{scope.row.businessDay | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="businessNo"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <span v-show="scope.row.payType==3">银行卡</span>
                <span v-show="scope.row.payType==4">会员</span>
                <span v-show="scope.row.payType==5">老会员</span>
                <span v-show="scope.row.payType==6">现金</span>
              </template>
            </el-table-column>
            <el-table-column label="支付金额" prop="sum" align="center">
            </el-table-column>
  ...
  methods:{
    ...
    exportTable(){
      for (let i = 0; i < this.tableData.length; i++) {
        this.tableData[i].businessDay = parseTime(this.tableData[i].businessDay,'{y}-{m}-{d}')
        this.tableData[i].createTime = parseTime(this.tableData[i].createTime)
        if (this.tableData[i].payType==1) {
          this.tableData[i].payType1 = '微信'
        }else if (this.tableData[i].payType==2) {
          this.tableData[i].payType1 = '支付宝'
        }else if (this.tableData[i].payType==3) {
          this.tableData[i].payType1 = '银行卡'
        }else if (this.tableData[i].payType==4) {
          this.tableData[i].payType1 = '会员'
        }else if (this.tableData[i].payType==5) {
          this.tableData[i].payType1 = '老会员'
        }else if (this.tableData[i].payType==6) {
          this.tableData[i].payType1 = '现金'
        }
      }
      var name = '营业额统计报表'
      var tHeaders1 = ['营业日', '交易流水号', '支付方式', '支付金额', '交易时间'];
      var filterVals1 = ['businessDay', 'businessNo', 'payType1', 'sum', 'createTime'];
