            ...
            <div class="biao publicTable thColorDark">
                <div class="table">
                    <el-table :data="tableData" style="width: 100%">
                        ...
                        <el-table-column prop="orderNo" label="交易流水号" width='270px;' align="center"></el-table-column>
                    ...
                    <div class="edit">
                        <div class="top">
                            <div class="box">
                                ...
                                <div class="block">
                                    <span>加油时间：{{seeData.detail.create_time | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
                                </div>
                                <div class="block">
                                    <span>交易时间：{{seeData.detail.cash_time | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
                                </div>
                                <div class="block">
                                    <span>车主类型：个人</span>
                                </div>
                            ...
                            <div class="box">
                                ...
                                <div class="block">
                                    <span>会员名称：{{seeData.detail.username}}</span>
                                </div>
                                <div class="block">
                                    <span>会员等级：{{seeData.detail.level_name}}</span>
                                </div>
                                
                                <div class="block">
                                    <span>会员积分：{{seeData.detail.current_point}}</span>
                                </div>
                                <div class="block">
                                    <span>会员编号：{{seeData.detail.member_no}}</span>
                                </div>
                            </div>
                            <div class="box">
                                <div class="block">
                                ...
                                <div class="block">
                                    <span>油枪号：{{seeData.detail.oil_gun_no}}</span>
                                </div>
    ...
    methods: {
        get() {
            ...
            getUserInfo(a)
                ...
                var a = {
                    id:response.data.belong_id
                }
                getOilsStationInfo(a)
                    .then(response => {
                        console.log('油站')
                        console.log(response.data)
                        // stationInfo
                        this.fullName = response.data.stationInfo.fullName
                    })
                var b = {
                    companyId: response.data.companyId,
                    stationId: response.data.belong_id
                };
                getDeviceListByStation(b).then(response => {
                    console.log('收银机');
                    console.log(response.data);
                    this.youqiangOptions = response.data;
                });
                getOilgunList(b).then(response => {
                    console.log('油枪号');
                    console.log(response.data);
