<template>
    <div class="app-container oil-business">
        <el-form ref="form" :model="form" label-width="100px">
            ...
            <el-table :data="list" fit highlight-current-row style="width: 100%;">
                ...
                <el-table-column label="操作时间" min-width="150" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.auditorTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                    </template>
                </el-table-column>
        ...
        <!--油品采购-->
        <el-dialog :visible.sync="shangpintank" width="50%" title="油品采购" class="caigousq_tab">
            <el-form label-width="110" label-position="left">
                <div class="mian_ord">
                    <div class="shang">
                        ...
                        <el-form-item
                            label="所属营业日："
                            v-if="zhuxinxi.businessDay"
                        >{{zhuxinxi.businessDay | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</el-form-item>
                        <el-form-item label="所属班次：" v-if="zhuxinxi.shiftId">{{zhuxinxi.shiftId}}</el-form-item>
                        <el-form-item
                            label="订单审核时间："
                            v-if="zhuxinxi.merchantAuditTime&&zhuxinxi.status==6"
                        >{{zhuxinxi.merchantAuditTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</el-form-item>
                        <el-form-item label="状态：" v-if="zhuxinxi.status==5">未收油</el-form-item>
                        <el-form-item label="状态：" v-if="zhuxinxi.status==6">已完成</el-form-item>
                    </div>
                    <div class="xinxi">
                        <ul class="dade">
                            <li>
                               <div class="title" style="width: 100%;font-weight: bold;color: red;font-size: 15px;">油品单信息：</div>
                                <div class="neirong">
                                    <ul class="limiande" v-if="youchuku">
                                        ...
                                        <li><div>出库密度 : <span>{{youchuku.primaryDensity}}升</span></div><div>出库重量 : <span>{{youchuku.primaryWeight}}kg</span></div></li>
            ...
            <el-form label-width="110" label-position="left">
                <div class="mian_ord">
                    <div class="shang guan">
                        ...
                        <el-form-item v-show="dbzhuxinxi.status!=1" label="调出油站：">{{dbzhuxinxi.oilsStationName}}</el-form-item>
                        <el-form-item label="调入油站：">{{dbzhuxinxi.inputOilsStationName}}</el-form-item>
                        <el-form-item label="调拔油品：">{{dbzhuxinxi.oilsName}}</el-form-item>
                        <el-form-item label="计划调拔体积：">{{dbzhuxinxi.planCount}}升</el-form-item>
                        <el-form-item
                            label="溢耗体积："
                        ...
                        <el-form-item
                            label="调入体积："
                            v-if="dbzhuxinxi.status==6"
                        >{{dbzhuxinxi.oilsOrderRecord.actualVolume}}升</el-form-item>
        ...
        <!--油品倒罐-->
        <el-dialog :visible.sync="youpindao" width="40%" title="油品倒罐" class="caigousq_tab">
            <el-form label-width="110" label-position="left">
                <div class="mian_ord">
                    <div class="shang">
                        <el-form-item
                            label="所属营业日："
                            v-if="dgzhuxinxi.businessDay"
                        >{{dgzhuxinxi.businessDay | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</el-form-item>
                        <el-form-item label="所属班次：" v-if="dgzhuxinxi.shiftId">{{dgzhuxinxi.shiftId}}</el-form-item>
                        <el-form-item
                            label="订单审核时间："
                            v-if="dgzhuxinxi.createTime"
                        >{{dgzhuxinxi.createTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</el-form-item>
                        <el-form-item label="状态：" v-if="dgzhuxinxi.status==0">未倒罐</el-form-item>
                        <el-form-item label="状态：" v-if="dgzhuxinxi.status==6">已完成</el-form-item>
                    </div>
                    <div class="xinxi">
                        <div class="shang guan" style="padding-top: 15px;">
                            ...
                            <el-form-item label="油品倒罐单号：">{{dgzhuxinxi.orderNumber}}</el-form-item>
                            <el-form-item label="油品：">{{dgzhuxinxi.oilsName}}</el-form-item>
                            <el-form-item label="倒罐体积：">{{dgzhuxinxi.actualVolume}}升</el-form-item>
                            <el-form-item label="损耗：">{{dgzhuxinxi.lossVolume}}升</el-form-item>
                            <el-form-item label="倒出罐号：">{{dgzhuxinxi.outOilcanNo}}号罐</el-form-item>
                            <el-form-item label="倒入罐号：">{{dgzhuxinxi.intoOilcanNo}}号罐</el-form-item>
                        </div>
                        <div class="guan" v-for="(item,index) in tableData5[0]">
                            <div class="title">
                                <div
                                    style="width: 100%;font-weight: bold;color: red;font-size: 15px;"
                                    v-if="index==0"
            ...
            <el-form label-width="110" label-position="left">
                <div class="mian_ord">
                    <div class="shang" style="border: none;">
                        <el-form-item
                            label="换号时间："
                        >{{hhxinxi.createTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</el-form-item>
                        <el-form-item label="换号罐号：">{{hhxinxi.oilcanNo}}</el-form-item>
                        <el-form-item label="换前油品：">{{hhxinxi.oldOilsName}}</el-form-item>
                        <el-form-item label="换后油品：">{{hhxinxi.newOilsName}}</el-form-item>
                        <el-form-item label="换号体积：">{{hhxinxi.changeVolume}}升</el-form-item>
                    </div>
                </div>
            </el-form>
