  ...
  <div class="app-container oil-business">
    <el-form ref="form" :model="form" label-width="100px">
      ...
      <div class="items">
        ...
        <el-form-item label="供应商" label-width="90px">
          <el-select v-model="listQuery.supplierId" clearable style="min-width: 200px" class="filter-item" @change="zhuangtaiX">
            <el-option v-for="item in gongyingshang" :key="item.id" :label="item.supplierShort" :value="item.id"/>
          </el-select>
        </el-form-item>
      ...
      <el-table :data="list" fit highlight-current-row style="width: 100%;">
        ...
        <el-table-column label="加油站" min-width="100" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" align="center" width="140" class-name="small-padding fixed-width">
          <template slot-scope="scope">
          	<!--采购的-->
            <el-button size="mini" type="success" v-if="scope.row.typeId==1" @click="cgDetail(scope.row,scope.row.typeId)">查看</el-button>
          	<!--调拨的-->
          	<el-button size="mini" type="success" v-if="scope.row.typeId==2" @click="dbDetail(scope.row,scope.row.typeId)">查看</el-button>
          	<!--退货的-->
          	<el-button size="mini" type="success" v-if="scope.row.typeId==3" @click="thDetail(scope.row,scope.row.typeId)">查看</el-button>
          	<!--报废的-->
          	<el-button size="mini" type="success" v-if="scope.row.typeId==4" @click="bfDetail(scope.row,scope.row.typeId)">查看</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog :visible.sync="shangpintank" :title="tkBiaoti" class="shangpintank_tab" width="900px">
      <el-form label-width="80" label-position="left">
      	...
      	<div v-if="zcDetail.typeIDID==1&&zcDetail.orderType==2">
		        ...
		        <el-form-item label="状态：" v-if="zcDetail.status==5">
		          	待收货
		        </el-form-item>
		        <el-form-item label="状态：" v-if="zcDetail.status==6">
		          	已完成
		        </el-form-item>
		        
		        <el-form-item label="订单类型：" v-if="zcDetail.orderType==1">
		          	系统订单
		        </el-form-item>
		        <el-form-item label="订单类型：" v-if="zcDetail.orderType==2">无单收货</el-form-item>
		        <el-form-item label="供应商：">
		          	{{zcList[0][0].supplierShortName}}
		        </el-form-item>
		        <el-form-item label="加油站：">
		          {{zcDetail.oilsStationShortName}}
		        </el-form-item>
		        <el-form-item label="收货时间：">
		          	{{zcList[0][0].auditTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="营业日：">
		        		{{zcDetail.businessDay | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="班次：">
					...
					<el-table :data="zcList[0]" style="width: 100%;">
		        ...
		        <el-table-column label="生产日期" align="center" min-width="70">
		          <template slot-scope="scope">
		            <span v-if="scope.row.productionDate">{{scope.row.productionDate | parseTime('{y}-{m}-{d}')}}</span>
		          </template>
		        </el-table-column>
			...
			<div class="shanga" style="border-bottom: 1px solid #E4E4E4;">
		        ...
		        <el-form-item label="订单类型：" v-if="zcDetail.orderType==2">
		          	无单收货
		        </el-form-item>
		        <el-form-item label="加油站：">
		          {{zcDetail.oilsStationShortName}}
					...
					<ul style="margin: 0;padding: 0;">
				        ...
				        <el-form-item label="状态：" v-if="item[0].status==1">
				          	未收货
				        </el-form-item>
				        <el-form-item label="状态：" v-if="item[0].status==2">
				          	收货中
				        </el-form-item>
				        <el-form-item label="状态：" v-if="item[0].status==3">
				          	已完成
				        </el-form-item>
				        <el-form-item label="采购子订单：">
				          {{item[0].orderNumber}}
				        </el-form-item>
				        <el-form-item label="收货时间：">
				          {{item[0].receiveTime |  parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
				        </el-form-item>
				        <el-form-item label="营业日：">
				        	{{zcDetail.businessDay | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
				        </el-form-item>
      	...
      	<div v-if="sdbDetail.typeIDID==2">
			<div class="shanga">
				...
				<el-form-item label="状态：" v-if="sdbDetail.status==1">
				  	已出库
				</el-form-item>
				<el-form-item label="状态：" v-if="sdbDetail.status==3">
				  	已完成
				</el-form-item>
				<el-form-item label="出库加油站：">
					{{sdbDetail.oilsNameOut}}
		        </el-form-item>
		        <el-form-item label="入库加油站：">
		          {{sdbDetail.oilsNameIn}}
		        </el-form-item>
		        <el-form-item label="营业日：">
		        	{{sdbDetail.businessDay | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="班次：">
		        	{{sdbDetail.shiftId}}
		        </el-form-item>
		        <el-form-item label="出库审核时间：" v-if="sdbDetail.auditTime">
		          	{{sdbDetail.auditTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="收货审核时间：" v-if="sdbDetail.receiveTime">
		          	{{sdbDetail.receiveTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="订单类型：" v-if="zcDetail.orderType==1">
		          	系统订单
		        </el-form-item>
      	...
      	<div v-if="sthDetail.typeIDID==3">
			<div class="shanga">
		        ...
		        <el-form-item label="状态：" v-if="sthDetail.status==6">
		          	已完成
		        </el-form-item>
		        <el-form-item label="供应商：">
		          {{sthList[0].supplierShortName}}
		        </el-form-item>
				<el-form-item label="加油站：">
					{{sthDetail.oilsStationName}}
		        </el-form-item>
		        <el-form-item label="营业日：" v-if="sthDetail.businessDay">
		        	{{sthDetail.businessDay | parseTime('{y}-{m}-{d}')}}
		        </el-form-item>
		        <el-form-item label="班次：" v-if="sthDetail.shiftId">
		        	{{sthDetail.shiftId}}
		        </el-form-item>
		        <el-form-item label="申请人：">
					{{sthDetail.createrName}}
		        </el-form-item>
		        <el-form-item label="申请时间：" v-if="sthDetail.createTime">
		          {{sthDetail.createTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="审核人：" v-if="sthDetail.merchantAuditorName">
		          {{sthDetail.merchantAuditorName}}
		        </el-form-item>
		        <el-form-item label="审核时间：" v-if="sthDetail.merchantAuditTime">
		          {{sthDetail.merchantAuditTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}
		        </el-form-item>
		        <el-form-item label="拒绝原因：" v-if="sthDetail.blocRefuseReason">
		        	{{sthDetail.blocRefuseReason}}
		        </el-form-item>
					</div>	
...
import { SupplierList,businessOrderList,orderShenShen,diaoboDetail,returnedDetail } from '@/api/shangpin'
