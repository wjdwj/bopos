	...
	<div class="shangpinAdd">
		...
		<div class="tianjiade">
			<el-table :data="list1" fit ref="moviesTable" class="xuan" @select-all="handleSelectionChange" @select="onTableSelect">
		        ...
		        <el-table-column label="油品品号" min-width="100" align="center" >
		    ...
		    <div class="xuanle">
		     	<div class="yixuan">已经选择的油品</div>
		     	<ul>
		     		<li v-for="item in yixuanList">
		     			<div class="ming">{{ item.oils_no_type }}</div>
...
<script>
	import { orderCaigou,orderList,orderLei,orderShen,orderShenList,orderShenCan,orderShenShen,SupplierList,diaoboList } from '@/api/shangpin'
...
//				父级传来的值
		...
		methods: {
			...
			getList() {
				this.msg = this.message;
				oilList(this.splistQuery).then(res => {
					console.log(res)
		    ...
		    shangyeT(){
		    	...
		    	this.$emit('oilData',this.caigoudeList)
		    ...
		    mingchengS(){
		    	...
		    	orderList(this.splistQuery).then(res=>{
		    		this.list1=res.data.rows;
		    		this.total1=res.data.total;
		    		let arr=[];
			    	console.log(this.selectData);
